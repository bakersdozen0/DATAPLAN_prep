...
# ============================================================================
RAW_WIDE_DATA_CSV <- "C:/Users/james.baker/Forest Research/TW CBC-TBA-NextGenBritishConifers - Share/Sitka/Backwards Selected Fullsib P96-P99 experiments/Kielder 162/Kielder_162_Full_Data_With_Flags.csv"

PLOT_TYPE      <- "SINGLE"  # Options: "SINGLE" (evaluates Plot sequence within Blocks) or "MULTI" (Trees within Plots)

# FALLBACK GRID DIMENSIONS (Used only if spatial Prow/Ppos are missing from the data)
GRID_ROWS      <- 10
GRID_COLS      <- 10

BASELINE_TRAIT <- "Dm_09" 
TEST_TRAITS    <- c("Ht_05") 
...
for (TEST_TRAIT in TEST_TRAITS) {
  ...
  recommendations <- all_results %>%
    group_by(Plot) %>%
    mutate(Normal_Cor = Spearman_Cor[start_corner %in% c("top_left", "normal") & direction == "horizontal" & snake == FALSE], Normal_Valid = Is_Biologically_Valid[start_corner %in% c("top_left", "normal") & direction == "horizontal" & snake == FALSE]) %>%
    filter(Is_Biologically_Valid == TRUE) %>% arrange(desc(Spearman_Cor)) %>% slice(1) %>% ungroup() %>%
    ...
    select(Plot, Action_Required, Best_Start = start_corner, Best_Dir = direction, Best_Snake = snake, Best_Cor = Spearman_Cor, Normal_Cor, Sample_Size) %>% arrange(desc(Action_Required))
  ...
  for (p in unique(working_data$Plot)) {
    ...
    if (nrow(rec) == 1 && !str_detect(rec$Action_Required, "(?i)None")) {
      ...
      # Need layout mat dimensions again for the fix plot
      p_rows <- GRID_ROWS; p_cols <- GRID_COLS
      if (exists("plot_spatial_trees")) {
        sp_data <- plot_spatial_trees %>% filter(Plot == p)
  ...
  cor_before <- calc_cor(before_df); cor_after <- calc_cor(after_df)
  ...
  p_compare <- ggplot(plot_df, aes(x = Base_Val, y = Test_Val)) +
    ...
    labs(title = paste("Plot Traversal Correction:", BASELINE_TRAIT, "vs", TEST_TRAIT), subtitle = sprintf("Trial Correlations  ->  Raw: %s  |  Targeted Fix: %s", cor_before, cor_after), x = paste("Trusted Baseline:", BASELINE_TRAIT), y = paste("Suspected Trait:", TEST_TRAIT))
  ...
  if (exists("plot_spatial_trees")) {
    plot_bounds <- plot_spatial_trees %>% group_by(Plot) %>% summarize(min_R = min(Map_Row), max_R = max(Map_Row), min_P = min(Map_Pos), max_P = max(Map_Pos), Centroid_Row = mean(Map_Row), Centroid_Pos = mean(Map_Pos), .groups = "drop")
    
    all_plots <- unique(working_data$Plot)
    
    map_df <- recommendations %>%
      mutate(Plot = as.character(Plot)) %>%
      bind_rows(tibble(Plot = setdiff(all_plots, recommendations$Plot), Action_Required = "Not Evaluated (N < 3 or Grid Mismatch)")) %>%
      left_join(plot_bounds, by = "Plot") %>%
      filter(!is.na(Centroid_Row)) %>%
      mutate(Path_Type = case_when(
        str_detect(Action_Required, "Not Evaluated") ~ "Not Evaluated",
        str_detect(Action_Required, "None") ~ "Target (Normal)",
        TRUE ~ paste0(toupper(Best_Start), " ", toupper(Best_Dir), if_else(Best_Snake, " (Snake)", " (Typewriter)"))
      ))
    
    path_list <- list()
    for(i in 1:nrow(map_df)) {
      r <- map_df[i, ]
      if(r$Path_Type == "Not Evaluated") next
      ...
      for (k in 1:lines_to_draw) {
        if (dir == "horizontal") {
          ...
          if (snk && k %% 2 == 0) { vx <- c(vx, end_x, start_x); vy <- c(vy, cur_y, cur_y) } else { vx <- c(vx, start_x, end_x); vy <- c(vy, cur_y, cur_y) }
        } else { 
          cur_x <- start_x + (k - 1) * step_x
          if (snk && k %% 2 == 0) { vx <- c(vx, cur_x, cur_x); vy <- c(vy, end_y, start_y) } else { vx <- c(vx, cur_x, cur_x); vy <- c(vy, start_y, end_y) }
        }
    ...
    if(nrow(map_df) > 0) {
      ...
      if("Not Evaluated" %in% names(my_colors)) my_colors["Not Evaluated"] <- "#e74c3c"
      ...
      p_map <- ggplot() +
        ...
        {if(nrow(path_df) > 0) geom_path(data = path_df, aes(x = X, y = Y, color = Path_Type, group = Plot), arrow = arrow(length = unit(0.08, "inches"), type = "closed"), linewidth = 0.6)} +
        ...
        theme(legend.position = "bottom", panel.grid.major = element_line(color = "grey95"), panel.grid.minor = element_blank(), plot.title = element_text(face = "bold", size = 16))
