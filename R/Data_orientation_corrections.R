...
RAW_WIDE_DATA_CSV <- "C:/Users/james.baker/Forest Research/TW CBC-TBA-NextGenBritishConifers - Share/Sitka/High GCA Fullsib P85-P87 experiments/Arecleoch 12/Arecleoch_12_Full_Data_With_Flags.csv"
...
if (USE_CORRECTED_DATA && file.exists(helper_path)) {
  # Read the current helper quietly
  existing_helper <- suppressMessages(read_csv(helper_path, show_col_types = FALSE))
  
  # Check if any of our Test Traits are already in the helper
...
for (TEST_TRAIT in TEST_TRAITS) {
  ...
  if (target_surv_base %in% names(df_raw)) {
